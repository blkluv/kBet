    ...
    <span
      className={`px-3 py-1 border text-sm font-mono ${
        ok
          ...
          : "bg-transparent text-green-400/50 border-green-400/50"
      }`}
    >
      {label} {ok ? "ON" : "OFF"}
    </span>
...
function EdgeCard({
  ...
  return (
    <div
      className={`rounded-2xl p-5 shadow-sm border ${urgencyColors[urgency]} flex flex-col gap-3`}
    >
      ...
      <div className="text-xs text-gray-500">
        We do not place bets. This is information only. Consider fees and
        limits.
      </div>
    </div>
  );
...
/** Lightweight type for YES/NO quotes that don’t include full H2H fields */
type AnyQuote = Pick<
  Quote,
  "eventId" | "sport" | "marketType" | "outcome" | "book" | "decimal"
> &
  Partial<Quote>;
...
const DEMO_NEWS = [
  {
    title: "NBA Finals Odds Shift After Injury Report",
    source: "ESPN",
    time: "2m ago",
    category: "sports",
    summary:
      "Lakers odds moved from +150 to +120 after Heat's star player injury announcement.",
  },
  {
    title: "Federal Reserve Rate Decision Impact on Markets",
    source: "Reuters",
    time: "15m ago",
    category: "economics",
    summary:
      "Prediction markets show 78% probability of rate hold, affecting economic futures.",
  },
  {
    title: "Election Polling Update: Swing State Analysis",
    source: "FiveThirtyEight",
    time: "1h ago",
    category: "politics",
    summary: "Latest polls show tightening race in key battleground states.",
  },
  {
    title: "AI Regulation Bill Progress",
    source: "TechCrunch",
    time: "2h ago",
    category: "tech",
    summary:
      "Senate committee advances AI safety legislation, prediction markets react.",
  },
];

export default function Page() {
  const [quotes, setQuotes] = useState<Quote[]>([]);
  const [yesno, setYesno] = useState<AnyQuote[]>([]);
  ...
  async function pull() {
    try {
      ...
      if (k.status === "fulfilled" && Array.isArray(k.value)) {
        y = y.concat(k.value as AnyQuote[]);
        status.kalshi = true;
      }
      if (p.status === "fulfilled" && Array.isArray(p.value)) {
        y = y.concat(p.value as AnyQuote[]);
        status.poly = true;
      }
      ...
      setYesno(DEMO_YESNO);
      setNews(DEMO_NEWS);
      setConnected({ odds: false, kalshi: false, poly: false, news: false });
    } finally {
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(pull, pollMs);
    return () => {
      if (timerRef.current) clearInterval(timerRef.current);
    };
  }, [pollMs]);
  ...
  const bestEdges = useMemo(() => {
    const events = groupBy(
      quotes.filter((q) => q.marketType === "h2h"),
      (q) => q.eventId
    );
    ...
    for (const [eventId, qs] of Object.entries(events)) {
      ...
      if (best) {
        const urgency =
          best.profitPct > 0.05 ? "high" : best.profitPct > 0.02 ? "medium" : "low";
  ...
  const crossIdeas = useMemo(() => {
    ...
    for (const [sport, qs] of Object.entries(grouped)) {
      const probs = (qs as Quote[])
        .map((q) => (q.decimal ? impliedProb(q.decimal) : NaN))
    ...
    for (const [sport, qs] of Object.entries(bySport)) {
      const probs = (qs as AnyQuote[])
        .map((q) => (q.decimal ? impliedProb(q.decimal) : NaN))
        .filter((x) => Number.isFinite(x)) as number[];
      if (!probs.length) continue;
      const pPred = probs.reduce((a, b) => a + b, 0) / probs.length;
      const pSport = sportAvgProb[sport] ?? NaN;
      if (Number.isFinite(pSport) && Number.isFinite(pPred)) {
        ideas.push({ sport, pSport, pPred, diff: Math.abs(pSport - pPred) });
      }
    }
  ...
  const marketStats = useMemo(() => {
    ...
    const avgProfit =
      bestEdges.length > 0
        ? bestEdges.reduce((sum, edge) => sum + edge.best.profitPct, 0) /
          bestEdges.length
        : 0;
              ...
              {bestEdges.length === 0 ? (
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {bestEdges.map(({ eventId, best, urgency }) => {
                    const title = `${best.h.home ?? "Home"} vs ${
                      best.h.away ?? "Away"
                    }`;
                    const subtitle = `${best.h.book} vs ${best.a.book}`;
                    const steps = [
                      `Bet $${best.s1.toFixed(2)} on ${
                        best.h.home ?? "Home"
                      } at ${best.h.book} (odds ${best.h.decimal.toFixed(2)})`,
                      `Bet $${best.s2.toFixed(2)} on ${
                        best.a.away ?? "Away"
                      } at ${best.a.book} (odds ${best.a.decimal.toFixed(2)})`,
                      `You lock in about ${toFixedPct(
                        best.profitPct
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold">
                        {(x.pSport * 100).toFixed(1)}%
                      </span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span>Prediction Markets:</span>
                      <span className="font-semibold">
                        {(x.pPred * 100).toFixed(1)}%
                      </span>
                    </div>
                  </div>
                  <div className="mt-4 pt-4 border-t border-gray-100">
                    <div className="text-emerald-700 text-lg font-bold">
                      Gap: {toFixedPct(x.diff)}
                    </div>
                    <div className="text-xs text-slate-500 mt-1">
                      Consider backing the cheaper side
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              {news.slice(0, 8).map((newsItem, i) => {
                const timeAgo = new Date(
                  newsItem.publishedAt || Date.now()
                ).toLocaleTimeString();
                const category = newsItem.category || "general";
                const source = newsItem.source || "Unknown";
                const summary =
                  newsItem.description || newsItem.title || "No description available";
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-medium ${
                          newsItem.analysis?.marketImpact === "high"
                            ? "bg-red-100 text-red-800"
                            : newsItem.analysis?.marketImpact === "medium"
                    ...
                    {newsItem.analysis?.tradingSignals &&
                      newsItem.analysis.tradingSignals.length > 0 && (
                        <div className="text-xs text-blue-600 bg-blue-50 p-2 rounded">
                          <strong>Signals:</strong>{" "}
                          {newsItem.analysis.tradingSignals[0]}
                        </div>
                      )}
          ...
          <section className="space-y-4">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
                ...
                <div className="font-semibold mb-2">We do not place bets.</div>
              ...
              <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
                ...
                <div className="text-sm text-gray-600">
                  Look for green cards above showing exact dollar amounts. We
                  calculate optimal bet sizes for $100 - adjust to your bankroll.
                </div>
              </div>
              <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200">
